...
import { createNewSection, type BlogFormSchema } from '@/pages/cms/create-blog/_components/article/blogForm.schema';
import { useEffect, useRef } from 'react';
import { useFieldArray, useFormContext } from 'react-hook-form';

export function useArticleSections() {
    const { control, setValue, getValues, formState } = useFormContext<BlogFormSchema>();
    ...
    const handleToggle = (index: number) => {
        ...
        sections.forEach((section, sectionIndex) => {
            ...
            setValue(
                `sections.${sectionIndex}.isExpanded`,
                willExpand ? false : section.isExpanded,
            );
        });
    };
