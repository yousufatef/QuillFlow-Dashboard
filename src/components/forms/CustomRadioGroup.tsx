...
type CustomRadioGroupProps<TFieldValues extends FieldValues = FieldValues> =
      ...
      >
        {options.map((option) => {
          ...
          return (
            <label
              className={cn(
                'flex cursor-pointer items-start gap-3 rounded-md border bg-background p-3 shadow-xs transition-colors hover:bg-accent/60 has-data-disabled:pointer-events-none has-data-disabled:opacity-50',
                optionClassName,
              )}
              htmlFor={optionId}
              key={option.value}
            >
              ...
              <span className='grid gap-1'>
                ...
                {option.description ? (
                  <span className={formFieldStyles.helper}>
                    {option.description}
                  </span>
                ) : null}
              </span>
