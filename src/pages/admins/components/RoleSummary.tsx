...
export default function RoleSummary({ username, email, role }: Props) {
  ...
  return (
    <CustomCard className='flex flex-col items-start gap-2'>
      ...
      <div className='text-muted-foreground space-y-2 break-all'>
        <p>
          {t('admin.form.lable.username')}: {username || 'N/A'}
        </p>
        <p>
          {t('table.email')}: {email || 'N/A'}
        </p>
      </div>
      ...
      {role ? (
        <>
          <p className='type-body-md mb-2 text-neutral-400'>
            {t('admin.role')}: {direction === 'ltr' ? role.nameEn : role.nameAr}
          </p>
