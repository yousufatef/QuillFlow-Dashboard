...
export const getBlogFormSchema = (
  t: TFunction,
  ...
  const sectionsSchema =
    isEditMode && !isPublished
      ? z.array(z.any()).max(MAX_BLOG_SECTIONS, `Maximum ${MAX_BLOG_SECTIONS} article sections are allowed`).optional()
      : z
...
export const getBlogFormDefaultValues = (blogData?: any): BlogFormSchema => {
  ...
  const sections = blogData.articles?.map((article: any) => ({
    ...
    isExpanded: true,
  })) ?? (blogData.isPublished === false ? [] : [{ id: crypto.randomUUID(), ...articleSectionDefaultValues }]);
